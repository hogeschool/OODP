...
public class Program
{
    ...
    /* 
        ...
        A Action delegate is a type that represents a method that takes zero or more input parameters and returns no value.
    */
    ...
    {
        List<Car> cars = new() { new Car("Kia", "Blue"), new Car("Mazda", "Red"), new Car("Volvo", "Black") };
        ...
        filteredCars.ForEach(car => Console.WriteLine($"{car.Brand} {car.Color}"));

        // SHORTEST (?)
        cars.FindAll(car => car.Brand.StartsWith("V")).ForEach(car => Console.WriteLine($"{car.Brand} {car.Color}"));
    }
